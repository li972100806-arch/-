...
public class JianghuEngine{
 ...
 public int mapIndex,skillLevel,equipmentPower,petLevel,petExp,sectLevel,artifactLevel,outfitLevel,towerFloor,travel;
 public long lastTime;private final Random random=new Random();
 public void load(Context c){SharedPreferences s=c.getSharedPreferences("jianghu_extra",Context.MODE_PRIVATE);mapIndex=Math.max(0,Math.min(maps.length-1,s.getInt("map",0)));skillLevel=Math.max(1,s.getInt("skill",1));equipmentPower=Math.max(0,s.getInt("equip",0));petLevel=Math.max(1,s.getInt("pet",1));petExp=Math.max(0,s.getInt("petExp",0));sectLevel=Math.max(1,s.getInt("sect",1));artifactLevel=Math.max(0,s.getInt("artifact",0));outfitLevel=Math.max(0,s.getInt("outfit",0));towerFloor=Math.max(0,s.getInt("tower",0));travel=Math.max(0,s.getInt("travel",0));lastTime=s.getLong("time",System.currentTimeMillis());}
 ...
 public boolean upgradeOutfit(Player p){int cost=(outfitLevel+1)*70;if(!pay(p,cost))return false;p.setSilver(p.getSilver()-cost);outfitLevel++;p.setMaxHp(p.getMaxHp()+8);return true;}
